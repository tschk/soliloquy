...
For mac-native loading, use `tools/soliloquy/start_macos.sh`. That path starts `sold`, serves the Svelte bundle, and launches Servo with `--no-browser-chrome` so the Crepuscularity-authored desktop shell is the only browser chrome. The old GPUI chrome is not the supported browser-engine path.
