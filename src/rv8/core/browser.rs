...
use super::{Tab, TabId, TabState, BrowserConfig, ProcessManager, NavigationController};
...
impl Browser {
    ...
    pub async fn new(config: BrowserConfig) -> Result<Self, String> {
        ...
        info!("Process manager initialized (multi_process={})", config.multi_process);
    ...
    pub async fn new_tab(&mut self, url: &str) -> Result<TabId, String> {
        let tab_id = TabId(self.next_tab_id.fetch_add(1, std::sync::atomic::Ordering::SeqCst));
