...
use soliloquy_browser_optimizations::memory::{
    compress, decompress, DiskStorage, FrozenTabState, TabResidencyManager,
};
...
fn main() {
    ...
    let mut disk_cache =
        DiskCache::new("./demo_cache", 100 * 1024 * 1024).expect("Failed to create disk cache");
...
fn freeze_idle_tabs(residency: &TabResidencyManager, storage: &mut DiskStorage) -> usize {
    ...
    for tab_id in 1u64..=tabs_to_freeze as u64 {
        ...
        let dom_snapshot = compress(&dom_data).expect("Compression failed");
        let render_snapshot = compress(&vec![0u8; 5000]).expect("Compression failed");
        let v8_snapshot = compress(&vec![0u8; 10000]).expect("Compression failed");
        ...
        storage
            .save_tab(&frozen)
            .expect("Failed to save frozen tab");
    }
...
fn demo_disk_cache(cache: &mut DiskCache) {
    ...
    cache
        .insert("shader:layout_compute", shader_code, "wgsl")
